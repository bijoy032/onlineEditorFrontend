...
// Require VITE_API_URL to be set in frontend/.env (or env-specific .env files)
const API_URL = import.meta.env.VITE_API_URL;
if (!API_URL) {
  console.error(
    "VITE_API_URL is not defined. Create frontend/.env with VITE_API_URL set to your backend URL."
  );
}
      ...
      <main className="p-4 flex gap-4 max-w-6xl mx-auto">
        ...
        <aside className="w-64 bg-white rounded shadow p-4">
          ...
          <div className="mb-4 p-3 bg-gray-50 rounded">
            ...
            {joinError && (
              <div className="text-red-500 text-xs mt-1">{joinError}</div>
            )}
            {joinSuccess && (
              <div className="text-green-500 text-xs mt-1">{joinSuccess}</div>
            )}
            ...
            <>
              ...
              <div className="mb-4 flex items-center gap-4">
                ...
                <span className="text-gray-500 text-sm">
                  (Collaborate face-to-face while editing!)
                </span>
              </div>
              {/* Placeholder for video call panel */}
              {inCall ? (
                <div className="flex flex-wrap gap-2 justify-center items-center">
                  {Object.entries(videoRefs.current).map(
                    ([peerId, videoEl]) => (
                      <video
                        key={peerId}
                        ref={(el) => {
                ...
                <span className="text-lg font-medium">
                  Video call panel will appear here
                </span>
              )}
